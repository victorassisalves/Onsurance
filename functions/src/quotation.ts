...
const vehicleType = this.get_field_value(10);
// console.log(`TCL: vehicleType`, vehicleType);
const usageType = this.get_field_value(11);
const usageTypePickup = this.get_field_value(123);
...
const executeCalculations = async (vehicleType: string) => {
        ...
        switch (vehicleType) {
            ...
            case "caminhonete": {
                ...
                } else if (usageType === "utilitario") {
                    minuteValueBase = calcMinuteVuc(fipe);;;
                } else {
                    throw {
                        errorType: "Tipo de uso inválido para caminhonete.",
                        message: `${usageType} não é válido. Só pode ser passeio ou utilitário.`,
                    };
