...
import { policesProtection } from "./polices";
import { getItemId } from "../database/database";


export const protectionMethods = async (protectionData, variables) => {
    ...
    const turnOn = [];
    const turnOff = [];
    await diffProtectionArray.forEach( element => {
    ...
    const initiateNewProtection = async () => {
        ...
        await userDbMethods.updateDatabaseInfo(userDbPath.child(`items/${itemDbId}`), {
            activationsCounter: activationsCounterUser
        });
        ...
        if (userProfile.activationsCounter === 0){ // Checks First Activation
            ...
            if (protectionData.lastProtection.closed){
                ...
                if (_.isEmpty(turnOff)) return activationResponse // No protection OFF Just ON but not all
                if (_.isEmpty(turnOn)) return deactivationResponse // No protection ON, just OFF but not All
                if (!_.isEmpty(turnOn) && !_.isEmpty(turnOff)) return deactivationActivationResponse // Protection ON and OFF
            }
        };
    };
    ...
    const closeProtection = async () => {
        ...
        await itemDbMethods.updateDatabaseInfo(itemDbPath.child(`logUse/${protectionData.lastProtectionId}`), closeLog);
        ...
        if (variables.statusProtection.allOff){
            ...
            return await initiateNewProtection()
        }
    }
